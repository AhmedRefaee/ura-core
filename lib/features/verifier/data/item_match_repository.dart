...
class ItemMatchRepository {
  ...
  Future<AppResult<ItemMatchResult>> matchAudio(Uint8List audioBytes, String mimeType) async {
    try {
      ...
      logger.d('ItemMatchRepository → roundTripMs: $roundTripMs, serverTiming: ${data['debug_timing_ms']}');
      final result = ItemMatchResult.fromMap(data);
      logger.i(
        'ItemMatchRepository → ${result.matches.length} matches, ${result.unmatched.length} unmatched',
      );
      return AppSuccess(result);
    } catch (e, st) {
      logger.e('ItemMatchRepository → matchAudio failed', error: e, stackTrace: st);
